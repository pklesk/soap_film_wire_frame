...
def experiment_hash_str(experiment_info, c_props, g_props, all_hs_digits=10, experiment_hs_digits=5, env_hs_digits=3):
    ...
    suffix += f"{experiment_info['NUMPY_SINGLE_THREAD']};{experiment_info['CONTRACTION_EPS']};{experiment_info['MC_SEED']};{experiment_info['MC_SAMPLES']};"
    for key in experiment_info.keys():
        if key.startswith("APPROACH_"):
